...
int next_var_num;
int next_label_num;
int find_var_num(const string&);
bool find_var_is_global(const string&);
%}
...
programbody     : variable_declarations
                {
                    for (auto& sym : sym_table.front()) {
                        if (sym.kind == VAR_K) {
                            fout << code_global_var(sym) << endl;
                        }
                    }
                }
                ...
                {
                    next_var_num = 1;
                    fout << code_main(filename);
                }
                compound_statement
                {
                    // code_inspect($5.code);
                    fout << code_gen($5.code);
                    fout << code_end();
                }
                ;
...
sign_literal_constant   : literal_constant
                        | MINUS INTEGER_LITERAL { $2.attr.ival *= -1; $$ = $2; }
                        | MINUS REAL_LITERAL { $2.attr.ival *= -1; $$ = $2; }
...
function_declaration    :
                        identifier
                        {
                            ...
                            func.name = $1.name;
                            func.kind = FUNC_K;
                            func_valid = check_valid(func) && func.type.type != ARR_T;
                        }
                        LEFT_PARENTHESES
                        {
                            next_var_num = 0;
                        ...
                        SEMICOLON compound_statement
                        {
                            // code_inspect($11.code);
                            sym_table.front().back().code = move($11.code);
                        }
                        END identifier
                        {
                            in_func = false;
                            func.attr.params.clear();
                            if (func.name != $14.name) error("the end of the functionName mismatch");
                            if (func_valid) {
                                fout << code_function(sym_table.front().back());
                            } 
                        }
                        ;
...
statements      : statement statements { $$.code = code_statements($1, $2); }
                |
                ;
...
simple_statement        : variable_reference ASSIGNMENT expression SEMICOLON
                        {
                            ...
                            if ($1.type.type != ERR_T) {
                                if ($1.kind == LOOP_VAR_K) {
                                    ...
                                    } else if ($1.type.type != $3.type.type and not ($1.type.type == REAL_T and $3.type.type == INT_T)) {
                                        error("type mismatch, LHS= " + type_str($1.type) + ", RHS= " + type_str($3.type));
                                    } else {
                                        if ($1.kind == VAR_K and find_var_is_global($1.name)) {
                                            $$.code = code_assignment_global($1, $3, filename);
                                        } else {
                                            $$.code = code_assignment($1, $3, find_var_num($1.name));
                                        }
                                    }
                                }
                            }
                        }
                        | PRINT expression SEMICOLON { check_print($2); $$.code = code_print($2); }
                        | READ variable_reference SEMICOLON { check_read($2); $$.code = code_read($2, filename, find_var_is_global($2.name)); }
                        ;
...
expression      : LEFT_PARENTHESES expression RIGHT_PARENTHESES { $$ = $2; }
                | MINUS expression %prec unary { $$ = $2; code_neg($$); }
                | expression ADD expression { $$ = check_arithmetic($1, $3, '+'); $$.code = code_arithmetic($$, $1, $3, '+'); }
                | expression MINUS expression { $$ = check_arithmetic($1, $3, '-'); $$.code = code_arithmetic($$, $1, $3, '-'); }
                | expression MUL expression { $$ = check_arithmetic($1, $3, '*'); $$.code = code_arithmetic($$, $1, $3, '*'); }
                | expression DIV expression { $$ = check_arithmetic($1, $3, '/'); $$.code = code_arithmetic($$, $1, $3, '/'); }
                | expression MOD expression { $$ = check_mod($1, $3); $$.code = code_mod($$, $1, $3); }
                | expression LT expression { $$ = check_cmp($1, $3, "<"); $$.code = code_cmp($1, $3, "<", next_label_num++); }
                | expression LE expression { $$ = check_cmp($1, $3, "<="); $$.code = code_cmp($1, $3, "<=", next_label_num++); }
                | expression EQU expression { $$ = check_cmp($1, $3, "="); $$.code = code_cmp($1, $3, "=", next_label_num++); }
                | expression GE expression { $$ = check_cmp($1, $3, ">="); $$.code = code_cmp($1, $3, ">=", next_label_num++); }
                | expression GT expression { $$ = check_cmp($1, $3, ">"); $$.code = code_cmp($1, $3, ">", next_label_num++); }
                | expression NE expression { $$ = check_cmp($1, $3, "<>"); $$.code = code_cmp($1, $3, "<>", next_label_num++); }
                | NOT expression { $$ = check_not($2); $$.code = code_not($2); }
                | expression AND expression { $$ = check_bool($1, $3, "and"); $$.code = code_bool($1, $3, "and"); }
                | expression OR expression { $$ = check_bool($3, $3, "or"); $$.code = code_bool($1, $3, "or"); }
                | variable_reference
                {
                    $$ = $1; $$.kind = RVAL_K;
                    if ($1.kind == VAR_K and find_var_is_global($1.name)) {
                        $$.code = code_var_ref_global($1, filename); 
                    } else {
                        $$.code = code_var_ref($1, find_var_num($1.name));
                    }
                }
                | procedure_call { $$ = $1; $$.kind = RVAL_K; code_proc_call($$, filename); }
                | literal_constant { $$ = $1; $$.kind = RVAL_K; $$.code.push_back(code_literal_constant($$)); }
                ;
...
conditional_statement   : IF expression
                        ...
                        {
                            $$.code = code_if($2, $5, $6, next_label_num++);
                        }
                        ;

else_clause     : ELSE statements { $$ = $2; }
                |
                ;
...
while_statement : WHILE expression
                ...
                {
                    $$.code = code_while($2, $5, next_label_num++);
                }
                ;
...
for_statement   : FOR identifier
                {
                    auto ident = $2;
                    ident.type.type = INT_T;
                    ident.kind = LOOP_VAR_K;
                    ident.var_num = next_var_num++;
                    check_valid_loop_var(ident);
                    loop_vars.push_back(ident);
                }
                ASSIGNMENT INTEGER_LITERAL TO INTEGER_LITERAL
                {
                    if ($5.attr.ival < 0 or $7.attr.ival < 0) error("lower or upper bound of loop parameter < 0");
                    else if ($5.attr.ival > $7.attr.ival) error("loop parameter's lower bound > uppper bound");
                }
                DO statements END DO
                {
                    $$.code = code_for(find_var_num($2.name), next_label_num, $5.attr.ival, $7.attr.ival, $10);
                    next_label_num++;
                    loop_vars.pop_back();
                }
                ;
                    ...
                    if (ok) {
                        ...
                        if (sym.attr.params.size() < cur_param_list.size()) {
                            ...
                            if (param_error) {
                                ...
                                auto& cur_param_list = param_list.back();
                                for (int i = 0; i < sym.attr.params.size(); ++i) {
                                    $$.code.splice($$.code.end(), cur_param_list[i].code);
                                    if (cur_param_list[i].type.type == INT_T and sym.attr.params[i].type.type == REAL_T) {
                                        $$.code.push_back("i2f");
                                    }
                                }
                            }
...
int find_var_num(const string& name) {
    for (auto& var : loop_vars) {
        if (var.name == name) return var.var_num;
    }
    for (auto scope_it = sym_table.rbegin(); scope_it != sym_table.rend(); ++scope_it) {
        for (auto& var : *scope_it) {
            if (var.name == name) return var.var_num;
        }
    }
    return -1;
...
bool find_var_is_global(const string& name) {
    for (auto& var : loop_vars) {
        if (var.name == name) return false;
    }
    for (auto scope_it = sym_table.rbegin(); next(scope_it) != sym_table.rend(); ++scope_it) {
        for (auto& var : *scope_it) {
            if (var.name == name) return false;
        }
    }
